...
export async function createGhnShipping(order: any) {
  ...
  const toPhone =
    order.metadata?.phone || order.shipping_address?.phone || "0987654321";
  ...
  const toWardCode = String(
    order.shipping_address?.metadata?.ward_code ||
      order.shipping_address?.ward ||
      "20308",
  );
  const toDistrictId =
    parseInt(
      order.shipping_address?.metadata?.district_id ||
  ...
  const fee =
    result?.total_price ||
    (order.metadata?.shipping_fee ? parseInt(order.metadata.shipping_fee) : 0);
  return {
