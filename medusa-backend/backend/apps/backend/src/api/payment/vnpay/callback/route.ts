...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    if (verify.isSuccess && vnp_ResponseCode === '00' && vnp_TxnRef) {
      console.log(`[VNPay Callback] ✅ Payment SUCCESS for order: ${vnp_TxnRef}`)
      ...
      if (paycol) {
        ...
        if (existingPaymentRes.rows.length === 0) {
          ...
          console.log(`[VNPay Callback] Registered payment records successfully for order: ${realOrderId}`)
        }
        ...
        console.log(`[VNPay Callback] DB updated payment_status=paid for order ${realOrderId}`)
      } else {
        console.warn(`[VNPay Callback] No payment collection found for order: ${realOrderId}`)
      }
      ...
      try {
        ...
        console.warn("[VNPay Callback] Event emit skipped:", eventErr.message)
      }
    } else {
      console.log(`[VNPay Callback] ❌ Payment FAILED or verification failed for order: ${vnp_TxnRef}`)
    }
