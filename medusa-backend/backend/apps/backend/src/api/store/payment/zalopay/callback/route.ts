  ...
  try {
    ...
    if (isSuccess && appTransId) {
      ...
      try {
        ...
        // Extract real Medusa order ID if formatted YYMMDD_order_xxx
        const realOrderId = appTransId.includes("order_")
          ? "order_" + appTransId.split("order_")[1]
          : appTransId;

        // 1. Update sprylo_order table if exists
        await db.raw(
          `UPDATE sprylo_order
          ...
          [checksum || appTransId, realOrderId, appTransId]
        );

        // 2. Update Medusa payment_collection if exists
        ...
        if (paycolRes.rows && paycolRes.rows.length > 0) {
