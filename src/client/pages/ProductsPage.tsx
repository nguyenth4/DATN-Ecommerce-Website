...
import { Star, Heart, ChevronRight, ChevronLeft, Search } from "lucide-react";
...
const ProductsPage = () => {
  ...
  const categories = (categoriesData || []).filter(
    (c: any) => (c.name || "").toLowerCase() !== "điện thoại",
  );
  ...
  const filteredAndSortedProducts = useMemo(() => {
      ...
      list = list.filter((p: any) => {
        ...
        if (variant?.calculated_price) {
          ...
          const origAmt = Number(
            variant.calculated_price.original_amount ?? pPrice,
          );
    ...
    if (sortBy === "views") {
      list.sort(
        (a, b) =>
          Number(b.metadata?.view_count || b.metadata?.views || 0) -
          Number(a.metadata?.view_count || a.metadata?.views || 0),
      );
    } else if (sortBy === "sales") {
      list.sort(
        (a, b) =>
          Number(b.metadata?.sale_count || b.metadata?.sales || 0) -
          Number(a.metadata?.sale_count || a.metadata?.sales || 0),
      );
    } else if (sortBy === "rating") {
      list.sort(
        (a, b) =>
          Number(b.metadata?.rating || 0) - Number(a.metadata?.rating || 0),
      );
      ...
      list.sort((a, b) => {
        const priceA =
          a.variants?.[0]?.prices?.find((pr: any) => pr.currency_code === "vnd")
            ?.amount ||
          ...
          0;
        const priceB =
          b.variants?.[0]?.prices?.find((pr: any) => pr.currency_code === "vnd")
            ?.amount ||
            ...
            >
              <button
                ...
                style={{
                  background: "var(--paper)",
                  border:
                    selectedCats.length === 0 && !isSaleFilter
                  ...
                  fontSize: "13px",
                  fontWeight:
                    selectedCats.length === 0 && !isSaleFilter ? "600" : "500",
                  color:
                    selectedCats.length === 0 && !isSaleFilter
                      ? "var(--indigo)"
                      : "var(--fg)",
              ...
              {categories.map((cat: any) => (
                <button
                  ...
                  style={{
                    background: "var(--paper)",
                    border: selectedCats.includes(cat.id)
                      ? "1px solid var(--indigo)"
                      : "1px solid var(--border)",
                    borderRadius: "8px",
                    cursor: "pointer",
                    ...
                    color: selectedCats.includes(cat.id)
                      ? "var(--indigo)"
                      : "var(--fg)",
                    padding: "8px 16px",
                    transition: "all 0.2s",
              ...
              {isLoadingProducts && allProducts.length === 0 ? (
                    ...
                    return paginatedProducts.map((p: any) => {
                      ...
                      if (variant?.calculated_price) {
                        ...
                        const saleP =
                          variant?.prices?.find(
                            (pr: any) =>
                              pr.currency_code === "vnd" && pr.price_list_id,
                          ) ||
                      ...
                      const displayPrice =
                        typeof pPrice === "number" && pPrice > 0
                          ? pPrice.toLocaleString("vi-VN") + "đ"
                          : "Liên hệ";
                      const displayOldPrice =
                        oldPrice && oldPrice > pPrice
                      ...
                      const hasAnyUnmanaged = allVariants.some(
                        (v: any) => v.manage_inventory === false,
                      );
                      const allNullInventory =
                      ...
                      const rawStock = allVariants.reduce(
                        (acc: number, v: any) => {
                      ...
                      const isInStock =
                        hasAnyUnmanaged || allNullInventory || stock > 0;
                        ...
                        <article className="product-card" key={p.id}>
                          <div className="img-wrap">
                            ...
                            <button
                              ...
                              style={{
                                opacity: wishlist.includes(p.id)
                                  ? 1
                                  : undefined,
                          ...
                          <div className="price">
                            ...
                            {displayOldPrice && (
                              <span className="was">{displayOldPrice}</span>
                            )}
                          </div>
                          <div className="stars">
                            {ratingCount > 0 ? (
                              ...
                              <input
                                ...
                                onChange={() => {
                                  ...
                                  if (!compareList.includes(p.id)) {
                                    ...
                                    toast("Đã gỡ khỏi danh sách so sánh", {
                                      icon: "🗑️",
                                    });
                                  }
                                }}
                ...
                >
                  ...
                  >
                    ...
                    >
                      Rất tiếc, chúng tôi không tìm thấy sản phẩm nào khớp với
                      bộ lọc của bạn. Hãy thử thay đổi từ khóa hoặc bộ lọc khác.
                    </p>
                    <button
